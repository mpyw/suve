...
func TestNewServer(t *testing.T) {
	...
	handler := func(_ *protocol.Request) *protocol.Response {
		return &protocol.Response{Success: true}
	}
	callback := func(_ *protocol.Request, _ *protocol.Response) {}
	shutdownCb := func() {}

	s := NewServer(handler, callback, shutdownCb)
	require.NotNil(t, s)
...
func TestServer_handleConnection_validRequest(t *testing.T) {
	...
	s := NewServer(handler, callback, nil)
...
func TestServer_handleConnection_shutdownCallback(t *testing.T) {
	...
	callback := func(_ *protocol.Request, resp *protocol.Response) {
...
//
//nolint:paralleltest // Uses t.Context() which is not compatible with t.Parallel().
...
func TestServer_Start_CreateSocketDirError(t *testing.T) {
	...
	// On Linux, socketPath uses XDG_RUNTIME_DIR first, then fallback to /tmp
...
func TestServer_Start_ListenError(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { _ = os.RemoveAll(tmpDir) })
	// On Linux, socketPath uses XDG_RUNTIME_DIR first
...
func TestServer_Start_ListenErrorLongPath(t *testing.T) {
	...
	// (Unix sockets have ~104-108 byte limit)
	longPath := tmpDir
	for range 10 {
