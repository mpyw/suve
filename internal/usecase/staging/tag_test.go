...
func TestTagUseCase_Tag_StagedForCreate_ResourceNotFound(t *testing.T) {
	...
	tagEntry, err := store.GetTag(t.Context(), staging.ServiceParam, staging.EntryKey{Name: "/app/new-param"})
	require.NoError(t, err)
	assert.Equal(t, "prod", tagEntry.Add["env"])
...
func TestTagUseCase_Untag_StagedForCreate_WithExistingTags(t *testing.T) {
	...
	require.NoError(t, store.StageTag(t.Context(), staging.ServiceParam, staging.EntryKey{Name: "/app/new-param"}, staging.TagEntry{
		Add:      map[string]string{"env": "prod", "team": "backend"},
		StagedAt: time.Now(),
	}))
...
func TestTagUseCase_Untag_StagedForCreate_AutoSkip(t *testing.T) {
	...
	_, err = store.GetTag(t.Context(), staging.ServiceParam, staging.EntryKey{Name: "/app/new-param"})
