...
func TestCommand_StagedValidation(t *testing.T) {
	...
	t.Run("staged without args", func(t *testing.T) {
		t.Parallel()
		app := appcli.MakeApp()
		err := app.Run(context.Background(), []string{"suve", "ssm", "diff", "--staged"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "usage: suve ssm diff --staged <name>")
	})

	t.Run("staged with version specifier", func(t *testing.T) {
		t.Parallel()
		app := appcli.MakeApp()
		err := app.Run(context.Background(), []string{"suve", "ssm", "diff", "--staged", "/app/config#1"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "--staged requires a parameter name without version specifier")
	})
	...
	t.Run("staged with too many args", func(t *testing.T) {
		t.Parallel()
		app := appcli.MakeApp()
		err := app.Run(context.Background(), []string{"suve", "ssm", "diff", "--staged", "/app/config", "/other/config"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "usage: suve ssm diff --staged <name>")
	})
}
...
func TestRunStaged_NotStaged(t *testing.T) {
	...
	err := r.Run(t.Context(), ssmdiff.Options{
		Staged:     true,
		StagedName: "/app/config",
	})
	require.NoError(t, err)
	assert.Contains(t, stderr.String(), "/app/config is not staged")
}
