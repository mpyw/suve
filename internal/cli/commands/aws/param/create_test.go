...
func TestCreateRun_WriteOptions(t *testing.T) {
	...
	t.Run("set flags produce options", func(t *testing.T) {
		...
		r := &cmdparam.CreateRunner{UseCase: &param.CreateUseCase{Writer: store}, Stdout: &buf, Stderr: &errBuf}
		err := r.Run(t.Context(), cmdparam.CreateOptions{
			Name:  "/app/param",
			Value: "v",
			Type:  "String",
	...
	t.Run("unset flags produce no options", func(t *testing.T) {
		...
		r := &cmdparam.CreateRunner{UseCase: &param.CreateUseCase{Writer: store}, Stdout: &buf, Stderr: &errBuf}
		err := r.Run(t.Context(), cmdparam.CreateOptions{Name: "/app/param", Value: "v", Type: "String"})
		require.NoError(t, err)
...
func TestCreateRun(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts:    cmdparam.CreateOptions{Name: "/app/param", Value: "test-value", Type: "String"},
