...
func TestUpdateRun_WriteOptions(t *testing.T) {
	...
	t.Run("set flags produce options", func(t *testing.T) {
		...
		r := &cmdparam.UpdateRunner{UseCase: &param.UpdateUseCase{Store: store}, Stdout: &buf, Stderr: &errBuf}
		err := r.Run(t.Context(), cmdparam.UpdateOptions{
			Name:  "/app/param",
			Value: "v",
			Type:  "String",
			ParamOpts: cmdparam.WriteOptionFlags{
	...
	t.Run("unset flags produce no options", func(t *testing.T) {
		...
		r := &cmdparam.UpdateRunner{UseCase: &param.UpdateUseCase{Store: store}, Stdout: &buf, Stderr: &errBuf}
		err := r.Run(t.Context(), cmdparam.UpdateOptions{Name: "/app/param", Value: "v", Type: "String"})
		require.NoError(t, err)
...
func TestUpdateRun_PreserveType(t *testing.T) {
	...
	r := &cmdparam.UpdateRunner{UseCase: &param.UpdateUseCase{Store: store}, Stdout: &buf, Stderr: &errBuf}
	err := r.Run(t.Context(), cmdparam.UpdateOptions{Name: "/app/secret", Value: "v", PreserveType: true})
	require.NoError(t, err)
...
func TestUpdateRun(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts:    cmdparam.UpdateOptions{Name: "/app/param", Value: "test-value", Type: "String"},
