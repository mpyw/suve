...
const (
	...
	// defaultDeleteRecoveryWindow is the AWS Secrets Manager default recovery window
	// in days, used to compute the displayed scheduled deletion date when no
	// explicit window is given.
	defaultDeleteRecoveryWindow = maxDeleteRecoveryWindow
)
...
func validateDeleteFlags(force bool, recoveryWindow int) error {
	...
	if recoveryWindow > 0 && (recoveryWindow < minDeleteRecoveryWindow || recoveryWindow > maxDeleteRecoveryWindow) {
		return fmt.Errorf("--recovery-window must be between %d and %d days", minDeleteRecoveryWindow, maxDeleteRecoveryWindow)
	}
   ...
   suve aws secret delete --yes my-secret                Delete without confirmation`,
		Flags: []cli.Flag{
			...
			&cli.IntFlag{
				...
				Value: defaultDeleteRecoveryWindow,
			},
			&cli.BoolFlag{
				Name:  "yes",
