...
func TestUpdateRun(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "update secret",
			opts: cmdsecret.UpdateOptions{Name: "my-secret", Value: "new-value"},
			store: &providermock.Store{
		...
		},
		{
			...
			opts: cmdsecret.UpdateOptions{Name: "my-secret", Value: "new-value", Description: "updated description"},
			store: &providermock.Store{
				GetFunc: func(_ context.Context, _ string, _ provider.VersionRef) (*domain.Entry, error) {
					return &domain.Entry{Value: "old-value"}, nil
				},
