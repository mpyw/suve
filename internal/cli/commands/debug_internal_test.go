// White-box tests for app.go's debug wiring, which is core, so this is too.
//declscope:core
...
func TestEnableDebug_aliasSummary(t *testing.T) {
	...
	assert.Contains(t, stderr, "flat aliases: param=aws secret=gcloud stage=(none) (AWS via ~/.aws/credentials or ~/.aws/config fallback)")
}
