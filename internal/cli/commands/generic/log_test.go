...
func TestLogCommand_Validation(t *testing.T) {
	t.Parallel()
...
func TestCommand_MaxValueLengthFlagReachesOptions(t *testing.T) {
	...
	logCmd := generic.LogCommand(generic.LogConfig{
		Usage:      "u",
		ArgsUsage:  "<name>",
		UsageError: "usage",
		Flags: []cli.Flag{
		...
		NewPresenter: func(_ context.Context, _ generic.LogRequest) (generic.LogPresenter, error) {
			return rec, nil
		},
	})
...
func runParamLog(
	t *testing.T, store *providermock.Store, req generic.LogRequest, opts generic.LogOptions,
) (string, error) {
	t.Helper()
	...
	r := &generic.LogRunner{Presenter: presenter, Options: opts, Stdout: &buf, Stderr: &errBuf}
	err := r.Run(t.Context())
...
func TestLogRunParam(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			req:  generic.LogRequest{Name: "/app/param", MaxResults: 10},
			opts: generic.LogOptions{MaxValueLength: 20},
			store: paramLogStore([]paramLogVer{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := runParamLog(t, tt.store, tt.req, tt.opts)
...
//nolint:funlen // Table-driven test with many cases
func TestLogRunSecret(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			req:  generic.LogRequest{Name: "my-secret", MaxResults: 10},
		...
		{
			name: "reverse order shows oldest first",
			req:  generic.LogRequest{Name: "my-secret", MaxResults: 10, Reverse: true},
			opts: generic.LogOptions{Reverse: true},
			store: secretLogStore([]domain.Version{
		...
		{
			name: "filter by since date",
			req:  generic.LogRequest{Name: "my-secret", MaxResults: 10, Since: at(now, -30*time.Minute)},
			store: secretLogStore([]domain.Version{
		...
		{
			name: "filter by until date",
			req:  generic.LogRequest{Name: "my-secret", MaxResults: 10, Until: at(now, -30*time.Minute)},
			store: secretLogStore([]domain.Version{
				{ID: "new-version", Labels: []string{"AWSCURRENT"}, Created: &now},
				{ID: "old-version", Labels: []string{"AWSPREVIOUS"}, Created: at(now, -2*time.Hour)},
			}, nil, nil, nil),
		...
		},
		{
			...
			req:  generic.LogRequest{Name: "my-secret", MaxResults: 10, Since: at(now, time.Hour)},
			store: secretLogStore([]domain.Version{
				{ID: "v2", Created: &now},
				{ID: "v1", Created: at(now, -time.Hour)},
			}, nil, nil, nil),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := runSecretLog(t, tt.store, tt.req, tt.opts)
