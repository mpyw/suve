// show.go provides the generic show command shared by every provider.
//
...
// metadata field layout / JSON shape — lives behind the ShowPresenter, so each
...
// ShowConfig holds the provider-specific configuration for the show command.
type ShowConfig[S any] struct {
...
// ShowCommand returns the generic show command wired with the provider ShowConfig.
func ShowCommand[S any](cfg ShowConfig[S]) *cli.Command {
	return &cli.Command{
		Name:        "show",
		Usage:       cfg.Usage,
