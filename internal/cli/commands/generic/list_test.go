...
// runParamList runs the generic ListRunner over the SSM Parameter Store usecase.
func runParamList(
	t *testing.T, store *providermock.Store, input ucparam.ListInput, opts generic.ListOptions,
) (string, error) {
	t.Helper()

	uc := &ucparam.ListUseCase{Reader: store}
	lister := func(ctx context.Context) ([]generic.ListEntry, error) {
	...
	r := &generic.ListRunner{List: lister, Options: opts, Stdout: &buf, Stderr: &errBuf}
	err := r.Run(t.Context())

	return buf.String(), err
}

// runSecretList runs the generic ListRunner over the Secrets Manager usecase.
func runSecretList(
	t *testing.T, store *providermock.Store, input ucsecret.ListInput, opts generic.ListOptions,
) (string, error) {
	t.Helper()
...
func TestListRunParam(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			opts:  generic.ListOptions{Show: true},
		...
		},
		{
			...
			opts:  generic.ListOptions{Output: output.FormatJSON},
			store: paramListStore([]string{"/app/param1", "/app/param2"}, nil, nil),
		...
		},
		{
			...
			opts:  generic.ListOptions{Output: output.FormatJSON, Show: true},
			input: ucparam.ListInput{WithValue: true},
			store: paramListStore(
				[]string{"/app/param1"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := runParamList(t, tt.store, tt.input, tt.opts)
...
func TestListRunSecret(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := runSecretList(t, tt.store, tt.input, tt.opts)
