// log.go provides the generic log command shared by every provider.
//
...
// line formatting to the provider LogPresenter so output stays byte-identical.
...
// LogRequest holds the version-history fetch parameters (passed to the usecase).
...
// LogRunner executes the log command over a provider LogPresenter.
...
// rendering.
func (r *LogRunner) Run(ctx context.Context) error {
...
// LogPatchParent computes, for version i in a fetched window of n versions, the
...
func LogPatchParent(i, n int, reverse bool) (parentIdx int, oldest bool) {
	if reverse {
		return i - 1, i == 0
	}
...
// LogConfig holds the provider-specific configuration for the log command.
type LogConfig struct {
	...
	Flags []cli.Flag
	// NewPresenter builds the provider LogPresenter bound to the fetch request.
	NewPresenter func(ctx context.Context, req LogRequest) (LogPresenter, error)
}

// LogCommand returns the generic log command wired with the provider LogConfig.
func LogCommand(cfg LogConfig) *cli.Command {
	return &cli.Command{
		Name:        "log",
