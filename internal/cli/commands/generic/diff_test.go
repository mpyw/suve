...
func TestDiffCommand_Validation(t *testing.T) {
	t.Parallel()
...
func assertParamSpec(t *testing.T, label string, got *version.NumericSpec, want *paramWantSpec) {
	t.Helper()
...
func assertSecretSpec(t *testing.T, label string, got *version.OpaqueSpec, want *secretWantSpec) {
	t.Helper()
...
func TestDiffRunParam(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			spec1: paramVersionSpec(1), spec2: paramVersionSpec(2),
			opts: generic.DiffOptions{ParseJSON: true},
			store: paramDiffStore(map[string]*domain.Entry{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, _, err := runDiff(t, presenter, tt.opts)
...
func TestParamIdenticalWarning(t *testing.T) {
	...
	spec := &version.NumericSpec{Name: "/app/param", Absolute: version.NumericAbsolute{}}
	presenter := cmdparam.NewDiffPresenter(store, spec, spec)
	stdout, stderr, err := runDiff(t, presenter, generic.DiffOptions{})
	require.NoError(t, err)
...
func TestDiffRunSecret(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts: generic.DiffOptions{Output: output.FormatJSON},
			store: secretDiffStore(map[string]*domain.Entry{
				":AWSPREVIOUS": {Name: "my-secret", Value: "old-secret", Version: domain.Version{ID: "prev-version-id-long"}},
				":AWSCURRENT":  {Name: "my-secret", Value: "new-secret", Version: domain.Version{ID: "curr-version-id-long"}},
			}, nil),
...
func TestSecretIdenticalWarning(t *testing.T) {
	...
	spec := &version.OpaqueSpec{Name: "my-secret", Absolute: version.OpaqueAbsolute{}}
