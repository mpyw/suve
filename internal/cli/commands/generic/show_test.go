...
func TestShowCommand_Validation(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{"param missing name", []string{"suve", "param", "show"}, "usage: suve aws param show"},
		{"param invalid version spec", []string{"suve", "param", "show", "/app/param#"}, "must be followed by"},
		{"secret missing name", []string{"suve", "secret", "show"}, "usage: suve aws secret show"},
...
func runShow(
	t *testing.T, presenter generic.ShowPresenter, opts generic.ShowOptions,
) (string, error) {
	t.Helper()

	var buf, errBuf bytes.Buffer

	r := &generic.ShowRunner{Presenter: presenter, Options: opts, Stdout: &buf, Stderr: &errBuf}
	err := r.Run(t.Context())
...
func TestShowRunParam(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			spec: "/my/param",
			opts: generic.ShowOptions{ParseJSON: true},
			store: showStore(&domain.Entry{
				Name:     "/my/param",
		...
		},
		{
			...
			spec: "/my/param",
			opts: generic.ShowOptions{Raw: true},
			store: showStore(&domain.Entry{
				Name:     "/my/param",
				Value:    "raw-value",
				Version:  domain.Version{ID: "1"},
		...
		},
		{
			...
			spec: "/my/param",
			opts: generic.ShowOptions{ParseJSON: true, Raw: true},
			store: showStore(&domain.Entry{
				Name:     "/my/param",
				Value:    `{"zebra":"last","apple":"first"}`,
				Version:  domain.Version{ID: "1"},
		...
		},
		{
			...
			spec: "/my/param",
			opts: generic.ShowOptions{Output: output.FormatJSON},
			store: showStore(&domain.Entry{
				Name:     "/my/param",
				Value:    "test-value",
				Version:  domain.Version{ID: "1"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := runShow(t, presenter, tt.opts)
...
//nolint:funlen // Table-driven test with many cases
func TestShowRunSecret(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "show latest version",
			spec: &version.OpaqueSpec{Name: "my-secret"},
			store: showStore(&domain.Entry{
				Name:    "my-secret",
				Value:   "secret-value",
				Version: domain.Version{ID: "abc123", Labels: []string{"AWSCURRENT"}, Created: &now},
