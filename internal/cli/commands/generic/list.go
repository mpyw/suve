// list.go provides the generic list command shared by every provider.
//
// The output rendering (names-only, --show text, and JSON forms) is identical
// across providers; only the small per-provider ListConfig (help text, flag set,
...
// ListEntry is a provider-neutral list row: a name plus an optional value or error.
...
// ListOptions holds the shared list options.
type ListOptions struct {
	Show   bool
	Output output.Format
}

// ListJSONOutputItem represents a single item in JSON output.
type ListJSONOutputItem struct {
	Name  string  `json:"name"`
	Value *string `json:"value,omitempty"` // nil when error or not requested, pointer to distinguish from empty string
	Error string  `json:"error,omitempty"`
}

// ListConfig holds the provider-specific configuration for the list command.
type ListConfig struct {
	...
	NewList func(ctx context.Context, cmd *cli.Command, withValue bool) (func(context.Context) ([]ListEntry, error), error)
}

// ListRunner executes the list command over a provider-supplied entry source.
type ListRunner struct {
	List    func(ctx context.Context) ([]ListEntry, error)
	Options ListOptions
	Stdout  io.Writer
	Stderr  io.Writer
}
...
func (r *ListRunner) Run(ctx context.Context) error {
	...
	if r.Options.Output == output.FormatJSON && !r.Options.Show {
		items := lo.Map(entries, func(entry ListEntry, _ int) ListJSONOutputItem {
			return ListJSONOutputItem{Name: entry.Name}
		})
	...
	if r.Options.Output == output.FormatJSON {
		items := lo.Map(entries, func(entry ListEntry, _ int) ListJSONOutputItem {
			if entry.Error != nil {
				return ListJSONOutputItem{Name: entry.Name, Error: entry.Error.Error()}
			}

			return ListJSONOutputItem{Name: entry.Name, Value: entry.Value}
		})
...
// ListCommand returns the generic list command wired with the provider ListConfig.
func ListCommand(cfg ListConfig) *cli.Command {
	return &cli.Command{
		Name:        "list",
		Aliases:     []string{"ls"},
		Usage:       cfg.Usage,
