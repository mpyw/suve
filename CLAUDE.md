...
- **GUI** — `internal/gui` (Wails app) + `internal/gui/frontend` (Svelte + Playwright). `app.go` holds the `App`, its lifecycle and the store/parser/strategy resolution, `scope.go` the scope selection and hydration, and `staging*.go` the staging bindings by concern (DTOs, status, mutate, apply, transfer). The sidebar scope form is built from capability `scopeFields` through `frontend/src/lib/scopeFields.ts`.
