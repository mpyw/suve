...
- **Provider adapters** — `internal/provider/{aws,gcloud,azure}`: each cloud's SDK-backed implementation. AWS client init sits at `internal/provider/aws/infra`. `internal/provider/aws/paramtype` is the SDK-free SSM type vocabulary (`String`/`SecureString`/`StringList`) that the CLI, TUI and GUI share. `internal/provider/builtin` composes the registry with all three; CLI, GUI and TUI build from it.
...
- **CLI commands** — `internal/cli/commands/{aws,gcloud,azure}` for provider groups: one flat package per service with one file per subcommand (`create.go` exposes `CreateCommand()`, and so on). The provider-neutral scaffold is the flat `internal/cli/commands/generic` package (`show.go`, `log.go`, `diff.go`, `list.go`, `tag.go`, `untag.go`). Wiring is in `internal/cli/commands/app.go` and `internal/cli/commands/internal`.
- **Staging** — `internal/staging`: the reducer-based transition state machine (`transition/`) over a keychain-encrypted file store (`store/file`, the only backend). The versioned per-provider strategies share `versioned.go` (`versionedStrategy[H]` plus a per-provider hooks type); App Configuration's unversioned strategy stands alone.
