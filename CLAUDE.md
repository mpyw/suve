...
- **Provider seam** — `internal/provider`: the `Reader`/`Writer`/`Tagger`/`Store` interfaces, `Registry`, `Scope`, opaque `VersionRef`, errors, and `Target` (`target.go`: the ordered `{Label, Value}` segments every UI shows for "who am I connected to", from `Scope.Target()`; `binding.ResolveTarget` fills a pending AWS target from STS). Provider selection lives in `internal/provider/detect` (env-detected flat aliases) + `registry.go`.
...
- **CLI commands** — `internal/cli/commands/{aws,gcloud,azure}` for provider groups: one flat package per service (`aws/{param,secret}`, `gcloud/secret`, `azure/{param,secret}`) with one file per subcommand (`create.go` exposes `CreateCommand()`, and so on). Stage commands are one `stage.go` in each provider root (`aws/stage.go`, `gcloud/stage.go`, `azure/stage.go`), holding every staged service's config and subgroup plus the all-service commands. Each provider root package owns its group, its `Flat*Command` constructors for the env-detected aliases, and any group-level flag (`gcloud` owns `--project`). Each provider's context keys, store resolvers and staging-scope resolvers live in `internal/cli/commands/<cloud>/internal`. The provider-neutral scaffold is the flat `internal/cli/commands/generic` package (`show.go`, `log.go`, `diff.go`, `list.go`, `tag.go`, `untag.go`). Wiring is in `internal/cli/commands/app.go` and `internal/cli/commands/internal` (the shared registry `Store(ctx, scope, kind)` plus `StrategyFactory`/`ParserFactory`).
- **Staging** — `internal/staging`: the reducer-based transition state machine (`transition/`) over a keychain-encrypted file store (`store/file`, the only backend). Each per-provider strategy is one `<provider>_<service>.go` file (`aws_param.go`, `aws_secret.go`, `google_cloud_secret.go`, `azure_param.go`, `azure_secret.go`). The versioned ones share `versioned.go` (`versionedStrategy[H]` plus a per-provider hooks type); App Configuration's unversioned strategy stands alone. `internal/staging/cli` holds the shared stage command builders; its `global_*.go` files are the provider-wide (all-service) `stage status|diff|apply|reset` that AWS and Azure wire, built on the per-service use cases plus `usecase/staging.GlobalApplyUseCase` (every service is conflict-checked before any is applied).
- **Staging binding** — `internal/staging/binding`: the one per-(provider, kind) lookup that the CLI, GUI and TUI share for the version grammar (`SplitSpec`, which the GUI uses to split `name#VERSION`), the store-less parser, the staging strategy over a store, the scope that keys staging state (Azure splits its two services; AWS resolves the STS identity), and the App Configuration namespace override. Add a provider here, not in each UI.
- **Version parsers** — the flat `internal/version` package. Three grammars parse `#VERSION` / `:LABEL` / `~SHIFT`: `NumericGrammar` (integer versions), `OpaqueGrammar` (opaque ids, with or without staging labels) and `BareGrammar` (unversioned; the whole argument is the name). `products.go` binds each product to one: `version.AWSParameterStore`, `version.AWSSecretsManager`, `version.GoogleCloudSecretManager`, `version.AzureKeyVault`, `version.AzureAppConfiguration`. `spec.go` holds the parsed `Spec` every grammar returns, and `parse.go` (with `shift.go`) is the shared engine the grammars build their `Parse` on. Each versioned grammar's `Suffix` rebuilds the suffix that callers hand to the use cases, and every grammar's `Split` returns the name and that suffix in one call. The package knows nothing about the CLI: each `diff.go` under `internal/cli/commands/<cloud>/<service>` wraps `generic.ParseDiffArgs` (`internal/cli/commands/generic/diff.go`) with its grammar and usage string (`parseDiffArgs`).
- **Capabilities** — `internal/capability`: the provider×service matrix every UI gates controls on (`All`, plus `Provider`, `Service` and `DisplayName` lookups). Add a capability here instead of checking the provider in a UI.
- **GUI** — `internal/gui` (Wails app) + `internal/gui/frontend` (Svelte + Playwright). `app.go` holds the `App`, its lifecycle and the store/parser/strategy resolution, `scope.go` the scope selection and hydration, and `staging*.go` the staging bindings by concern (DTOs, status, mutate, apply, transfer). "Apply All" is `StagingApplyAll`, built on `usecase/staging.GlobalApplyUseCase` like the CLI's all-service apply. The sidebar scope form is built from capability `scopeFields` through `frontend/src/lib/scopeFields.ts`.
- **TUI** — `internal/tui`: the app shell (`app.go` update and key routing, `mouse.go`, `view.go`, `actions.go` for the dialogs and staging flows it opens) over subpackages for pages, dialogs and components. `internal/tui/data` is its data seam: `data.go` (the `Source` interface and shared types), `param_source.go`/`secret_source.go`, `mutator*.go`, and the staging seam `stage.go` (its `StagingApplyAll` runs the Staging tab's apply-all through `usecase/staging.GlobalApplyUseCase`, like the CLI's all-service `stage apply`).
...
- **Staging store**: keychain-encrypted (zalando/go-keyring), scope-keyed under `~/.suve/staging/{scope.Key()}/` with per-service `param.json`/`secret.json`; `SUVE_STAGING_KEY` overrides the keychain data key (used in CI/tests). Raw-key files (crypt format v3) bind `scope.Key()` and the file name as AES-GCM associated data; legacy v2 files are still read and re-encrypted on the next write.
...
`gui/` is a separate Go module (the Wails entry point) that replaces `github.com/mpyw/suve` with `../`. After a dependency change in the root `go.mod`, run `go mod tidy` in `gui/` too. declscope cannot load an untidy module.

declscope reads only the files the current build configuration selects. A file behind `//go:build` is never read, and a package whose files are all behind one is not listed at all. Its `-tags` flag does nothing: pass tags through `GOFLAGS`, and set `GOOS` for OS-specific files:
