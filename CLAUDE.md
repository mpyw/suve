...
- **GUI** — `internal/gui` (Wails app) + `internal/gui/frontend` (Svelte + Playwright). `app.go` holds the `App` and its lifecycle, `scope.go` the scope selection and hydration, `stores.go` the store/parser/strategy resolution, and `staging*.go` the staging bindings by concern (DTOs, status, mutate, apply, transfer). The sidebar scope form is built from capability `scopeFields` through `frontend/src/lib/scopeFields.ts`.
- **TUI** — `internal/tui`: the app shell (`app.go` update and key routing, `mouse.go`, `view.go`, `actions.go` for the dialogs and staging flows it opens) over subpackages for pages, dialogs and components. `internal/tui/data` is its data seam: `data.go` (the `Source` interface and shared types), `param_source.go`/`secret_source.go`, `mutator*.go`, and the staging `probe.go`/`staging.go`.
