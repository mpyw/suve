...
- **`<!-- site:skip --> … <!-- /site:skip -->`** is GitHub-only content dropped from the site — applied to both the README and every `docs/*.md` page. Use it for anything the site's left-nav already provides or that would be self-referential there: the README's "Documentation" link back to the site, and each docs page's top "Back to README | \<sibling docs\>" nav row (kept for direct GitHub browsing, omitted from the site).
